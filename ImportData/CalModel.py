...
class CalMethod:
 	...
 	def __corr(self,num,g):	
 		EXY = (g.iloc[:,1] * g.iloc[:,2]).rolling(window=num).mean()
 		EXEY = g.iloc[:,1].rolling(window=num).mean() * g.iloc[:,2].rolling(window=num).mean() 
 		stdXstdY = g.iloc[:,1].rolling(window=num).std() * g.iloc[:,2].rolling(window=num).std() 
 		g = EXY**2 - EXEY / stdXstdY
 		return g
 	...
 	def Corr(self,col1,col2,num): # 过去 num 天相关性，双变量
 		...
 		return  np.array(grouped.iloc[:,2].values.tolist()).flatten()
...
if __name__=='__main__':
	C=C()
	SLD = SD.SelectDataMatrix()
	Cal = CalMethod()
	ret = SLD.get(C.pctChg)
	clo = SLD.get(C.close)
	beta = Cal.Cut(clo,100)
	print(beta)
	f = ret * beta
	ff = pd.DataFrame(f)
	print(ff)
